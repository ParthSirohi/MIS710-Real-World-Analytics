...
##################################
data= read.delim('RedWine.txt')

dataset<- read.csv('RedWine.txt')
data.raw <- as.matrix(read.table("RedWine.txt"))


set.seed(224080369) # using your student ID number for reproducible sampling with the seed function
...
x1 = data.variable.names[,1]
y = data.variable.names[,5]
plot(data.variable.names[,1],data.variable.names[,2])
